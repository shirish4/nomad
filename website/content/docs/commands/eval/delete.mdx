...
the evaluation ID is omitted, this command will use the filter options below to
delete a set of evaluations.
...
- `-deployment`: Filter by an associated deployment ID. This option may be
  specified multiple times.

- `-node`: Filter by an associated node ID. This option may be specified multiple
  times.
...
```shell-session
$ nomad eval delete -status=pending -job=example
...
prompts:
```shell-session
$ nomad eval delete -scheduler=service -schduler=system -filter-operator=or -yes
