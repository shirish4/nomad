...
- `-preemption-batch-scheduler` - Specifies whether preemption for batch jobs
  is enabled. Note that if this is set to true, then batch jobs can preempt any
  other jobs. Must be one of `[true|false]`.

- `-preemption-service-scheduler` - Specifies whether preemption for service jobs
  is enabled. Note that if this is set to true, then service jobs can preempt any
  other jobs. Must be one of `[true|false]`.

- `-preemption-sysbatch-scheduler` - Specifies whether preemption for system batch
...
- `-preemption-system-scheduler` - Specifies whether preemption for system jobs
